...
[role="lead"]
NetApp Data Classification é um serviço de governança de dados no NetApp Console que analisa suas fontes de dados locais e de nuvem para mapear e classificar dados e identificar informações privadas. Ele ajuda você a reduzir riscos de segurança e conformidade, diminuir custos de storage e dar suporte a projetos de migração de dados.


IMPORTANT: A partir da versão 1.31, a Classificação de Dados está disponível como um recurso principal no NetApp Console. Não há custo adicional. Não é necessária licença ou assinatura para a Classificação de Dados. + Se você estiver usando a versão legada 1.30 ou anterior, essa versão estará disponível até o vencimento da sua assinatura.
...
O NetApp Console oferece gerenciamento centralizado de NetApp storage e serviços de dados em ambientes locais e em nuvem, em escala empresarial. O Console é necessário para acessar e usar os serviços de dados da NetApp. Como interface de gerenciamento, ele permite que você gerencie muitos recursos de storage a partir de uma única interface. Os administradores do Console podem controlar o acesso ao storage e aos serviços para todos os sistemas da empresa.
...
* Identifique informações de identificação pessoal (PII).
...
O NetApp Data Classification fornece ferramentas para ajudar a reduzir o custo de propriedade (TCO) do storage. Você pode usar o NetApp Data Classification para:
...
Os custos de transferência de dados dependem da sua configuração. Se a instância de NetApp Data Classification e a fonte de dados estiverem na mesma zona de disponibilidade e região, então não haverá custos de transferência de dados. Mas se a fonte de dados, como um sistema Cloud Volumes ONTAP, estiver em uma zona de disponibilidade ou região _diferente_, então você será cobrado pelo seu provedor de nuvem pelos custos de transferência de dados. Consulte estes links para mais detalhes:
...
* Na AWS, a Classificação de Dados é executada em um  https://aws.amazon.com/ec2/instance-types/m6i/["instância m6i.4xlarge"^] com um disco GP2 de 500 GiB. A imagem do sistema operacional é o Amazon Linux 2.
...
* **As varreduras completas** fornecem uma análise em nível profundo dos seus dados. Uma varredura completa inclui uma varredura somente de mapa e a classificação dos dados dentro dos arquivos.


Para uma análise detalhada das diferenças entre varreduras apenas com mapa e varreduras completas, consulte link:task-scanning-overview.html["Qual é a diferença entre varreduras de mapeamento e classificação?"].
