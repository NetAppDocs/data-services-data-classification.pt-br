...
summary: Com NetApp Data Classification, você pode reduzir os custos de armazenamento identificando dados obsoletos, arquivos duplicados e arquivos grandes, removendo-os ou organizando-os em storage de objetos menos dispendioso 
---
= Visualize detalhes de governança sobre dados organizacionais com NetApp Data Classification
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. Em cada bloco de Oportunidades de Economia do painel de Governança, selecione *Otimizar Armazenamento* para visualizar os resultados filtrados na página de Investigação. Para descobrir quaisquer dados que você deve excluir ou migrar para um armazenamento mais econômico, investigue as _Oportunidades de Economia_.
+
...
.Passos
. Em Classificação de Dados, selecione *Governança*.
. No bloco de relatórios, selecione o botão **Download** no cabeçalho do relatório Visão geral completa do mapeamento de dados.
+
