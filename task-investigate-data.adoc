...
---
= Investigue os dados da sua organização com NetApp Data Classification
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
O painel de Investigação de Dados exibe informações sobre seus dados em nível de arquivo e diretório, permitindo que você classifique e filtre os resultados. A página Investigação de Dados apresenta informações sobre metadados e permissões de arquivos e diretórios, além de identificar arquivos duplicados. Com informações em nível de arquivo, diretório e banco de dados, você pode tomar medidas para melhorar a conformidade da sua organização e economizar espaço de storage. A página Investigação de Dados também oferece suporte a link:task-copy-move.html["mover, copiar e excluir arquivos"].


NOTE: Para obter informações na página de Investigação, você precisa realizar uma análise completa de classificação em suas fontes de dados. Fontes de dados que passaram apenas por uma análise de mapa não exibem detalhes em nível de arquivo.
...
| Número de identificadores | Selecione o intervalo de identificadores sensíveis detectados por arquivo. Inclui dados pessoais e dados pessoais sensíveis. Ao filtrar em Diretórios, o Data Classification totaliza as correspondências de todos os arquivos em cada pasta (e subpastas). NOTA: A versão de dezembro de 2023 (1.26.6) removeu a opção de calcular o número de dados de informações de identificação pessoal (PII) por Diretórios. 
...
TIP: Após filtrar os dados, você pode link:task-copy-move.html["copiar, mover e excluir arquivos não estruturados"] para melhorar a conformidade ou otimizar o armazenamento.
...
* link:task-copy-move.html["Copie, mova e exclua dados do painel de investigação"]
