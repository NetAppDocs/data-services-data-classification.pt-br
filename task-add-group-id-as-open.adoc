...
summary: 'Quando IDs de grupo são associados a arquivos ou pastas em compartilhamentos de arquivos NFS, eles definem as permissões para o arquivo ou pasta. Se os IDs de grupo não estiverem configurados com o nível de permissão "Aberto para a organização", você pode adicionar essa permissão ao GID para que todos os arquivos e pastas que tiverem esse GID associado sejam considerados "abertos para a organização".' 
---
