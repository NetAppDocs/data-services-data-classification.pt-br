...
+
** A NetApp Data Classification não consegue extrair permissões ou a "última hora de acesso" de sistemas 7-Mode.
...
+
Para garantir que os "últimos horários de acesso" dos seus arquivos permaneçam inalterados pelas verificações do NetApp Data Classification, configure o usuário do Active Directory com permissões de Write Attributes no CIFS ou permissões de gravação no NFS. Se possível, configure o usuário do Active Directory como parte de um grupo pai na organização que tenha permissões para todos os arquivos.
...
NOTE: A página de visão geral do sistema possui uma opção *Escanear sem permissões de gravação* que está desativada por padrão. Se o Data Classification não tiver as permissões apropriadas, ele não escaneará os arquivos porque o Data Classification não pode reverter a "última hora de acesso" para o carimbo de data/hora original. + Você pode substituir essa configuração usando a opção *Escanear sem permissões de gravação*. Para saber mais sobre essa opção, consulte link:reference-collected-metadata.html#last-accessed-time-timestamp["Metadados coletados de fontes de dados na Classificação de Dados"].
...
Você pode iniciar ou interromper varreduras em qualquer sistema a qualquer momento no menu Configuração. Você também pode alternar varreduras de somente mapa para varreduras completas, e vice-versa.

A opção *Escanear sem permissões de gravação* está desativada por padrão. Isso significa que, se o Data Classification não tiver as permissões de gravação apropriadas, o sistema não escaneará os arquivos porque o Data Classification não poderá reverter o "horário do último acesso" para o registro original. Se o seu sistema puder tolerar a modificação do horário do último acesso, selecione esta opção. link:reference-collected-metadata.html#last-accessed-time-timestamp["Saber mais"^].


NOTE: Novos volumes adicionados ao sistema são verificados automaticamente somente quando você habilita a verificação para todos os volumes. Se você habilitou a verificação apenas em determinados volumes, você deve habilitar manualmente a verificação no volume recém-adicionado.
