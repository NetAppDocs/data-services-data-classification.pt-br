...
Observe que você também pode link:task-deploy-compliance-onprem.html["instalar a Classificação de Dados em um host Linux que tenha acesso à Internet"]. Esse tipo de instalação pode ser uma boa opção se você preferir analisar sistemas ONTAP locais usando uma instância do NetApp Data Classification que também está localizada localmente, mas isso não é um requisito. O software funciona exatamente da mesma maneira, independentemente do método de instalação escolhido.
...
[role="quick-margin-para"]
Certifique-se de que seu ambiente atenda aos pré-requisitos. Isso inclui acesso à internet de saída para a instância, conectividade entre o agente do Console e a Data Classification pela porta 443, entre outros. <<Pré-requisitos,Veja a lista completa>>.
...
Se você ainda não tem um agente do Console, crie um agente do Console em seu provedor de nuvem. Consulte  https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-aws.html["criando um agente de console na AWS"^] ou  https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-azure.html["criando um agente de console no Azure"^], ou  https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-google.html["criando um agente de console no GCP"^]. Na maioria dos casos, você provavelmente já tem um agente do Console configurado antes de tentar ativar o NetApp Data Classification porque a maioria  https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html["Os recursos do console exigem um agente do console"], mas há casos em que você precisa configurar um agora.

Existem alguns cenários em que você precisa usar um agente de Console implantado em um provedor de nuvem específico:
