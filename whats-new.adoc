...
=== Aumento dos limites para copiar e mover
...
Esta versão de classificação do BlueXP inclui alterações de código que otimizam a utilização de recursos.
...
=== Priorize apenas as varreduras de mapa

Ao realizar varreduras somente de Map, você pode priorizar as varreduras mais importantes. Esse recurso é útil quando você tem muitos ambientes de trabalho e deseja garantir que as varreduras de alta prioridade sejam concluídas primeiro.
...
A priorização é limitada a link:https://docs.netapp.com/us-en/data-services-data-classification/concept-classification.html["varreduras apenas de mapa"^]; não está disponível para varreduras de mapa e classificação.
...
Esta versão incluiu pequenas melhorias.

O lançamento também removeu as seguintes opções:

* A opção para ativar a coleta de logs de auditoria foi desabilitada.
* Durante a investigação dos diretórios, a opção para calcular o número de dados de informações de identificação pessoal (PII) por diretórios não está disponível. Consulte link:task-investigate-data.html["Investigue os dados armazenados em sua organização"].
...
As versões 8 e 9 do Red Hat Enterprise Linux não suportam o mecanismo Docker, que era anteriormente necessário para a instalação do BlueXP classification. BlueXP classification agora suporta instalação no RHEL 9.0, 9.1 e 9.2 usando o Podman versão 4 ou superior como infraestrutura de contêineres. Se o seu ambiente exigir o uso das versões mais recentes do RHEL, agora você pode instalar o BlueXP classification (versão 1.26 ou superior) usando o Podman.

Atualmente, o Data Classification não oferece suporte a instalações em dark sites nem a ambientes de varredura distribuída (usando um nó mestre e nós de varredura remotos) ao usar o RHEL 9.x.
...
=== Adicione palavras-chave personalizadas na interface de classificação do BlueXP
...
O filtro "Número de usuários com acesso" agora suporta arquivos armazenados em repositórios do SharePoint Online. Anteriormente, apenas arquivos em compartilhamentos CIFS eram suportados. Grupos do SharePoint que não são baseados no Active Directory não são contabilizados neste filtro no momento.
