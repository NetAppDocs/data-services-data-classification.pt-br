...
* Certifique-se de que haja uma conexão de rede entre a instância do NetApp Data Classification e cada rede que inclua volumes para o Cloud Volumes ONTAP ou clusters ONTAP locais.
...
NOTE: A NetApp Data Classification examina apenas um compartilhamento de arquivos em um volume. Se você tiver vários compartilhamentos em seus volumes, você precisará examinar esses outros compartilhamentos separadamente como um grupo de compartilhamentos. link:reference-limitations.html#data-classification-scans-only-one-share-under-a-volume["Veja mais detalhes sobre esta limitação de Classificação de Dados"^].
