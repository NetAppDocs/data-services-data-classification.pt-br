...
* Por padrão, se Data Classification não tiver permissões de gravação de atributos no CIFS ou permissões de gravação no NFS, o sistema não analisará os arquivos em seus volumes porque Data Classification não pode reverter o "último acesso" para o carimbo de data/hora original. Se você não se importa que o último acesso seja redefinido, selecione *Or select scanning type for each volume*. A página resultante tem uma configuração que você pode habilitar para que Data Classification analise os volumes independentemente das permissões.
