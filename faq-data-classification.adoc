...
---
= Perguntas frequentes sobre NetApp Data Classification
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Use este FAQ para encontrar respostas rápidas para perguntas comuns sobre NetApp Data Classification.
...
A Classificação de Dados implementa outra camada de IA juntamente com seu sistema NetApp Console e sistemas de armazenamento. Em seguida, ela examina os dados em volumes, buckets, bancos de dados e outras contas de armazenamento e indexa os insights de dados encontrados. A Classificação de Dados utiliza tanto inteligência artificial quanto processamento de linguagem natural, ao contrário de soluções alternativas que geralmente são baseadas em expressões regulares e correspondência de padrões.

O NetApp Data Classification utiliza IA para fornecer uma compreensão contextual dos dados para detecção e classificação precisas. É impulsionado por IA porque foi projetado para tipos de dados modernos e escala. Também entende o contexto dos dados para proporcionar descoberta e classificação precisas e eficazes.
...
A Classificação de Dados faz parte dos recursos principais do NetApp Console. Você não precisa implantar a Classificação de Dados a partir de um marketplace de nuvem.
...
Ao analisar dados, o agente do NetApp Console precisa ser instalado nos seguintes locais:

* Para Cloud Volumes ONTAP na AWS ou Amazon FSx para ONTAP: o agente do console está na AWS.
* Para o Cloud Volumes ONTAP no Azure ou no Azure NetApp Files: o agente do console está no Azure.
...
O NetApp Console permite que você analise e gere relatórios de sistemas em praticamente qualquer lugar, incluindo ambientes locais, em nuvem e híbridos. A Data Classification normalmente é implantada usando um modelo SaaS, no qual o serviço é habilitado por meio da interface do Console e não requer instalação de hardware ou software. Mesmo nesse modo de implantação com apenas alguns cliques, o gerenciamento de dados pode ser feito independentemente de os armazenamentos de dados estarem locais ou na nuvem pública.
...
* Na AWS, a Classificação de Dados é executada em uma instância m6i.4xlarge com um disco GP2 de 500 GiB.
...
.Posso migrar o agente do Console ou a instância do Data Classification para um host diferente em uma implantação de site escuro?
Não. Migrar o agente do Console para um host diferente não é compatível quando o NetApp Data Classification está implantado em um agente do Console. Se você precisar do NetApp Data Classification em um agente do Console diferente em um site isolado, implante um novo agente do Console e uma nova instância do NetApp Data Classification no novo host.



[[data-classification-management]]
== Gerenciamento de classificação de dados

As perguntas a seguir fornecem informações sobre como gerenciar o NetApp Data Classification.
...
Para remover completamente a instância do NetApp Data Classification, remova manualmente a instância do NetApp Data Classification no portal do seu provedor de nuvem ou no seu ambiente local.
...
Quando a Classificação de Dados examina volumes que possuem dados frios em camadas no storage de objetos usando varreduras somente de mapa, ela examina todos os dados--dados que estão em discos locais e dados frios em camadas no storage de objetos. Isso também é verdade para produtos não-NetApp que implementam o armazenamento em camadas.

A varredura somente do mapa não aquece os dados frios: eles permanecem frios e continuam no storage de objetos. Por outro lado, se você executar a varredura completa, algumas configurações podem aquecer os dados frios.



[[source-systems-and-data-types]]
== Sistemas de origem e tipos de dados
...
Quando o NetApp Data Classification detecta informações de identificação pessoal (PII) ou quando realiza uma pesquisa DSAR, somente os seguintes formatos de arquivo são suportados:

`+.CSV, .DCM, .DOC, .DOCX, .JSON, .PDF, .PPTX, .RTF, .TXT, .XLS, .XLSX, Docs, Sheets, and Slides+`

.Que tipos de dados e metadados a Classificação de Dados captura?
A NetApp Data Classification permite que você execute uma varredura somente de mapa ou uma varredura completa em suas fontes de dados. As varreduras somente de mapa fornecem apenas uma visão geral de alto nível dos seus dados, enquanto as varreduras completas fornecem uma análise detalhada dos seus dados. As varreduras somente de mapa podem ser feitas em suas fontes de dados muito rapidamente, pois não acessam arquivos para visualizar os dados internos.

* *Análise somente de mapas*: O NetApp Data Classification analisa apenas os metadados. Isso é útil para o gerenciamento e governança geral de dados, definição rápida do escopo de projetos, grandes ambientes e priorização. O mapeamento de dados é baseado em metadados e é considerado uma análise *rápida*.
+
...
* *Verificação completa*: A NetApp Data Classification analisa dados usando protocolos padrão e permissão somente leitura em todos os seus ambientes. Arquivos selecionados são abertos e verificados em busca de dados confidenciais relacionados a negócios, informações privadas e problemas relacionados a ransomware.
+
...
A Classificação de Dados captura metadados como: nome do arquivo, permissões, data de criação, último acesso e última modificação. Isso inclui todos os metadados que aparecem na página Detalhes da Investigação de Dados e nos Relatórios de Investigação de Dados.
...
O desempenho da verificação pode variar dependendo da largura de banda da rede e do tamanho médio dos arquivos em seu ambiente. Também pode depender do tamanho e da localização do sistema host (ou seja, se está na nuvem ou localmente). Consulte link:concept-classification.html["A instância de classificação de dados"] e link:task-deploy-overview.html["Implementando a Classificação de Dados"] para obter mais informações.

Ao adicionar novas fontes de dados pela primeira vez, você também pode optar por realizar apenas uma varredura de mapeamento ou uma varredura completa. O mapeamento pode ser feito em suas fontes de dados muito rapidamente, pois não acessa arquivos para visualizar os dados internos. link:task-scanning-overview.html["Entenda a diferença entre uma varredura de mapeamento e uma varredura de classificação"].
...
.Se eu reiniciar minha máquina virtual durante as verificações, qual é o impacto na verificação?
As verificações persistem. Se você reiniciar uma máquina virtual enquanto as verificações estiverem em execução, elas serão retomadas de onde pararam após a reinicialização bem-sucedida da máquina virtual.
...
As perguntas a seguir referem-se ao tratamento de dados privados e ao acesso a dados pelo NetApp Data Classification.
...
Não. Os dados privados enviados entre o seu navegador e a instância do Data Classification são protegidos com criptografia completamente usando TLS 1.2, o que significa que NetApp e terceiros não podem lê-los. O Data Classification não compartilha quaisquer dados ou resultados com NetApp a menos que você solicite e aprove o acesso.
