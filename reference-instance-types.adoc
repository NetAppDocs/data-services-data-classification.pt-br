---
sidebar: sidebar 
permalink: reference-instance-types.html 
keywords: BlueXP classification, installation, deployment, instance, virtual machine, cpu, ram, memory, disk, capacity 
summary: 'O software de classificação de dados deve ser executado em um host que atenda aos requisitos específicos do sistema operacional, requisitos de RAM, requisitos de software e assim por diante.  Ao implantar a Classificação de Dados na nuvem, é recomendável usar um sistema com características "grandes" para funcionalidade completa.' 
---
= Tipos de instância de NetApp Data Classification com suporte
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
:linkattrs: 
:imagesdir: ./media/


[role="lead"]
NetApp Data Classification deve ser executado em um host que atenda aos requisitos específicos de sistema operacional, requisitos de RAM e requisitos de software. Ao implantar Data Classification na nuvem, recomendamos que você use um sistema com as características "large" para funcionalidade completa.

Nas tabelas a seguir, se o sistema marcado como "default" não estiver disponível na região onde você está instalando o Data Classification, o próximo tipo de instância listado na tabela será implantado.



[[aws-instance-types]]
== Tipos de instância da AWS

[cols="20,30,25"]
|===
| Tamanho do sistema | Especificações | Tipo de instância 


| Extra grande | 32 CPUs, 128 GB de RAM, 1 TiB gp3 SSD | https://aws.amazon.com/ec2/instance-types/m6i/["m6i.8xlarge"^](padrão) 


| Grande | 16 CPUs, 64 GB de RAM, SSD de 500 GiB | https://aws.amazon.com/ec2/instance-types/m6i/["m6i.4xlarge"^](padrão) m6a.4xlarge m5a.4xlarge m5.4xlarge m4.4xlarge 
|===


[[azure-instance-types]]
== Tipos de instância do Azure

[cols="20,30,25"]
|===
| Tamanho do sistema | Especificações | Tipo de instância 


| Extra grande | 16 CPUs, 64 GB de RAM, disco do sistema operacional (2.048 GiB, taxa de transferência mínima de 250 MB/s) e disco de dados (1 TiB SSD, taxa de transferência mínima de 750 MB/s) | https://learn.microsoft.com/en-us/azure/virtual-machines/sizes/general-purpose/dv5-series["Standard_D32s_v5 ou Standard_D32_v5"^] 


| Grande | 16 CPUs, 64 GB de RAM, SSD de 500 GiB | link:https://learn.microsoft.com/en-us/azure/virtual-machines/sizes/general-purpose/dsv5-series?tabs=sizebasic["Standard_D16_v5 ou Standard_D16s_v5"^] (padrão) 
|===


[[gcp-instance-types]]
== Tipos de instância do GCP

[cols="20,30,25"]
|===
| Tamanho do sistema | Especificações | Tipo de instância 


| Grande | 16 CPUs, 64 GB de RAM, SSD de 500 GiB | https://cloud.google.com/compute/docs/general-purpose-machines#n2_machines["n2-padrão-16"^](padrão) n2d-standard-16 n1-standard-16 
|===