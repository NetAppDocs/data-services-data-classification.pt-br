...
---
= Instale o NetApp Data Classification em um host Linux no modo privado
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Use o _modo privado_ para instalar o NetApp Data Classification em um host Linux em um site local sem acesso à internet (também conhecido como uma instalação offline ou dark site). Essa implantação utiliza um script de instalação e mantém a instância do Data Classification desconectada da camada SaaS do NetApp Console, permitindo que você analise fontes de dados locais em um ambiente isolado.
...
O script de instalação do NetApp Data Classification verifica se o sistema e o ambiente atendem aos pré-requisitos necessários. Se atenderem, a instalação é iniciada. Se você quiser verificar os pré-requisitos independentemente da execução da instalação do NetApp Data Classification, pode baixar um pacote de software separado que testa apenas os pré-requisitos. link:task-test-linux-system.html["Veja como verificar se o seu host Linux está pronto para instalar o Data Classification"].
...
Quando instalado em modo privado, o Data Classification só pode analisar dados de fontes de dados que também são locais no site on-premises. No momento, o Data Classification pode analisar as seguintes fontes de dados *locais*:
...
A migração do agente do Console para um host diferente não é compatível quando o NetApp Data Classification está implantado em um agente do Console. Se você precisar do NetApp Data Classification em um agente do Console diferente em um site isolado, implante um novo agente do Console e uma nova instância do NetApp Data Classification no novo host.
...
* O NetApp Data Classification deve estar em um host dedicado. O host não pode ser compartilhado com outros aplicativos ou softwares de terceiros, como antivírus.
...
** *Tamanho da VM do Azure*: "Standard_D16_v5". link:reference-instance-types.html#azure-instance-types["Veja tipos adicionais de instância do Azure"^].
...
+
*** Red Hat Enterprise Linux versão 8.8, 8.10, 9.0, 9.1, 9.2, 9.3, 9.4, 9.5, 9.6 e 9.7.


** As extensões de vetor avançadas (AVX2) devem estar habilitadas no sistema host.


* *Gerenciamento de Assinaturas Red Hat*: O host deve estar registrado no Gerenciamento de Assinaturas Red Hat. Se não estiver registrado, o sistema não poderá acessar os repositórios para atualizar o software de terceiro necessário durante a instalação.
...
NOTE: O instalador de Data Classification reserva o UID 4321 e o GID 4321. Antes de iniciar a instalação, certifique-se de que o UID 4321 e o GID 4321 não estejam em uso na máquina host. Se estiverem em uso, a instalação falhará com uma mensagem de erro "invalid user".
...
Na maioria das situações, você instala o Data Classification em um sistema de host único. Este não é o mesmo host de máquina virtual que o agente do Console.

image:diagram_deploy_onprem_single_host_no_internet.png["Um diagrama que mostra a localização das fontes de dados que você pode analisar ao usar uma única instância de Data Classification implantada localmente sem acesso à internet."]
