...
. Na página de Configuração, localize o sistema StorageGRID para o qual deseja ativar a verificação.
...
.Resultado
A Classificação de Dados inicia a verificação dos buckets que você habilitou. Você pode acompanhar o progresso da verificação inicial acessando o menu **Configuração**. Identifique o sistema e selecione **Gerenciar**. O progresso de cada verificação é exibido como uma barra de progresso. Você também pode passar o cursor sobre a barra de progresso para ver o número de arquivos verificados em relação ao total de arquivos no volume. Se houver algum erro, ele aparecerá na coluna Status, juntamente com a ação necessária para corrigir o erro.
