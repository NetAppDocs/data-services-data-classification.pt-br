...
[role="lead"]
Utilize a NetApp Data Classification API REST para acessar as mesmas funcionalidades disponíveis na interface web.
...
** Os dados são exportados em formato JSON (fácil de analisar e enviar para aplicativos de terceiros, como Splunk)


* Crie consultas usando instruções "AND" e "OR", inclua e exclua informações e muito mais.
+
Por exemplo, você pode localizar arquivos _sem_ informações de identificação pessoal específicas (PII) (funcionalidade não disponível na interface do usuário). Você também pode excluir campos específicos da operação de exportação.
...
Para acessar o Swagger, você precisa do endereço IP da sua instância do NetApp Data Classification. Para uma implantação de nuvem, use o endereço IP público. Em seguida, você precisa acessar este endpoint:
...
.Solicitação de API
Você precisa obter todos os campos e opções relevantes para visualizar todos os filtros na guia Investigação do sistema.
...
----
Utilize a resposta nos parâmetros da solicitação para filtrar os arquivos que você deseja copiar.

Você pode aplicar uma ação em vários itens.  Os tipos de ação suportados incluem: mover, excluir e copiar.

Crie a ação de cópia:

.Solicitação de API
Esta API permite que você crie múltiplas ações.
